...
async def oauth(request: Request) -> HTTPResponse:
    ...
    return redirect(request.ctx.__dict__['session'].get('after_auth_redirect',
                                                        use_after_auth_default_redirect))
...
def login_required(async_handler=None, provider=None, add_user_info=True, email_regex=None):
    ...
    async def wrapped(request: Request, **kwargs):
        ...
        if 'token' not in request.ctx.__dict__['session']:
            if provider:
                request.ctx.__dict__['session']['oauth_provider'] = provider
            request.ctx.__dict__['session']['after_auth_redirect'] = request.path
            return redirect(oauth_endpoint_path)
